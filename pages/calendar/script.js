...
export default class Mt {
	...
	// LUNAL
	lunar_month_days = [1887, 0x1694, 0x16aa, 0x4ad5,
	...
	solar_1_1 = [1887, 0xec04c, 0xec23f, 0xec435, 0xec649,
	...
	SolarFromInt(g) {
		...
		let solarYear = parseInt(y);
		let solarMonth = parseInt(mm);
		let solarDay = parseInt(dd);
		return [solarDay, solarMonth, solarYear];
	};
	LunarToSolar(lunarDay, lunarMonth, lunarYear, isleap) {
		var days = this.lunar_month_days[lunarYear - this.lunar_month_days[0]];
		...
		var solar11 = this.solar_1_1[lunarYear - this.solar_1_1[0]];
	...
	SolarToLunar(solarDay, solarMonth, solarYear) {
		var index = solarYear - this.solar_1_1[0];
		var data = (solarYear << 9) | (solarMonth << 5)
			| (solarDay);
		...
		var m = this.GetBitInt(solar11, 4, 5);
		var d = this.GetBitInt(solar11, 5, 0);
		var offset = this.SolarToInt(solarYear, solarMonth, solarDay) - this.SolarToInt(y, m, d);
		...
		var lunarDay = parseInt(offset);
		let lunarYear = lunarY;
		let lunarMonth = lunarM;
		let isleap = false;
